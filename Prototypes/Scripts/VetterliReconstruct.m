function [QP_E, QP] = VetterliReconstruct(binary_image, x_a, x_b, y_a, y_b, x_n, y_n, psnr)
    ...
    %     binary_image = GetImageOfPowerPolynomial(GetRandomPowerPolynomial(4, x_a, x_b, y_a, y_b, false), x_a, x_b, y_a, y_b, x_n, y_n);
    %     binary_image = RandomStablyBounded4thDegreeBivariatePolynomial(x_a, x_b, y_a, y_b, x_n, y_n);
    ...
    poly_coefs_QP = quadprog(HH * 10000, ff, A_ineq(b_ineq ~= inf, :), b_ineq(b_ineq ~= inf), A_eq, b_eq, [], [], x0, options);
